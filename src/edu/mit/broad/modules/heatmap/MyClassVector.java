...
public class MyClassVector {
	...
	public MyClassVector(int size) {
		cv = new ColoredClassVector(size); // FIXME set default class label for
											// everything
	}
	 ...
	 * cv.addClass(classNumber, label); notifyListeners(); }
	 */

	/*
	 * public void setClass(int index, int classNumber) { if(classNumber !=
	 * DEFAULT_CLASS_NUMBER) { hasLabels = true; } cv.setClass(index,
	 * classNumber); notifyListeners(); }
	 */
	...
	public ClassVectorListener[] getListeners() {
		EventListener[] temp = eventListeners
				.getListeners(ClassVectorListener.class);
