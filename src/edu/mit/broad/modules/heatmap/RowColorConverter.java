...
public final class RowColorConverter {
	 ...
	 * 
	 * @param colormap
	 *            array of rgb values. Each element contains an RGB value
	 ...
	 *            Description of the Parameter
	 */
	public RowColorConverter(int[] colormap, int response,
			IExpressionData dataset) {
	...
	public RowColorConverter(Color[] colors, int response,
			IExpressionData dataset) {
		this.colors = colors;
	...
	public void setColorResponse(int _response) {
		if (_response != COLOR_RESPONSE_LINEAR
				&& _response != COLOR_RESPONSE_LOG) {
			throw new IllegalArgumentException("Unkown color response");
		}
	 ...
	 *            Description of the Parameter
	 */
	private void calculateSlots(final double min, final double max,
			final double mean, final double[] values) {
